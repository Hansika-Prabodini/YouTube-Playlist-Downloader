...
def main():
    ...
    while True:
        ...
        if playlist_url.lower() == 'exit':
            break
        if not playlist_url:
            print("Please enter a valid URL or type 'exit'.")
            continue
...
def fetch_playlist_info(url):
    ...
    try:
        ...
        for line in iter(process.stdout.readline, ''):
            if line.strip():
                try:
                    ...
                    title = video_json.get('title') or "Untitled"
                    video_url = video_json.get('url') or video_json.get('id')
                    # yt-dlp accepts bare IDs, but normalize to full URL if needed
                    if isinstance(video_url, str) and not video_url.startswith("http"):
                        video_url = f"https://www.youtube.com/watch?v={video_url}"
                    if video_url:
                        video_info_list.append({'title': title, 'url': video_url})
        ...
        if process.returncode not in (0, None):
            # If yt-dlp returned non-zero, surface a generic message
            print("Warning: yt-dlp returned a non-zero status while reading the playlist.")
